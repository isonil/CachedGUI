...
public static class CachedGUI
{
    ...
    private static int? prevCachedPartIDMouseover;
    	...
    	bool skipAllEvents = false)
	{
        ...
        if( dirtyOnMouseover )
        {
            ...
            if( rect.Contains(Event.current.mousePosition) || rect.Contains(GUIUtility.ScreenToGUIPoint(mousePos)) )
            {
                ...
                if( prevCachedPartIDMouseover != null && prevCachedPartIDMouseover != ID )
                {
                    // dirty previous group, with no mouseover anymore
                    for( int i = 0; i < cachedParts.Count; i++ )
                    {
                        if( cachedParts[i].ID == prevCachedPartIDMouseover )
                        {
                            var p = cachedParts[i];
                            p.dirty = true;
                            cachedParts[i] = p;
                ...
                prevCachedPartIDMouseover = ID;
            }
            else if( prevCachedPartIDMouseover == ID ) // no longer mouseover
            {
                wasDirty = true;
                prevCachedPartIDMouseover = null;
            }
        }
